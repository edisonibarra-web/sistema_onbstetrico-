...
class Command(BaseCommand):
    help = (
        "Ejecuta continuamente la sincronización de signos vitales "
        "desde Dinámica."
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"[CONTINUO] Iniciando sincronización automática "
                f"(intervalo objetivo: {intervalo:.1f}s)."
            )
        )
        ...
        while ciclos == 0 or ejecutados < ciclos:
            ...
            try:
                call_command(
                    "sincronizar_signos_vitales_dinamica",
                    verbosity=1,
                )
                ...
                self.stderr.write(
                    self.style.ERROR(
                        f"[CONTINUO] Error en sincronización: {exc}"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"[CONTINUO] Finalizado después de {ejecutados} ciclo(s)."
            )
        )
